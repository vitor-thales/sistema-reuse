...
export async function updateSenhaEmpresa(idEmpresa, hashNovo, keys) {
    try {
        await db.query(
            'UPDATE tbEmpresas SET senhaHash = ?, ikPrivada = ?, salt = ?, iv = ? WHERE idEmpresa = ?',
            [hashNovo, keys.privateKey, keys.salt, keys.iv, idEmpresa]
        );
        return true;
    } catch (err) {
        console.error(err);
